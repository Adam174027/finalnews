...
export async function getServerSideProps(context) {
    try {
      ...
      let users = await db.collection("test1").find({catagory:'Cricket'}).limit(9).sort({$natural:-1}).toArray();
      ...
      let videosResult = await db.collection("videos").find({}).limit(9).sort({_id:-1}).toArray();
